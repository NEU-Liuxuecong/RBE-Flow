...
class RoadScenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        'Mix'
        # visible_path = osp.join(self.root_dir,  self.mode, 'wrapped', '512_0Trans0.1Scale90Rot','image_pair', f'{pair_id}_visible.tif')
        # infrared_path = osp.join(self.root_dir,  self.mode, 'wrapped', '512_0Trans0.1Scale90Rot','image_pair', f'{pair_id}_infrared_warped.tif')
        # flow_path = osp.join(self.root_dir, self.mode, 'wrapped', '512_0Trans0.1Scale90Rot','truth_flow', f'{pair_id}.flo')
        "BigMixRoad"
        visible_path = osp.join(self.root_dir, self.mode, 'wrapped', '128_15Trans0.1Scale30Rot','image_pair', f'{pair_id}_rgb.tif')
        infrared_path = osp.join(self.root_dir, self.mode, 'wrapped','128_15Trans0.1Scale30Rot', 'image_pair', f'{pair_id}_ir_warped.tif')
        flow_path = osp.join(self.root_dir, self.mode, 'wrapped', '128_15Trans0.1Scale30Rot','truth_flow', f'{pair_id}.flo')
        # 'WHU'
        '''
        visible_path = osp.join(self.root_dir, self.mode, 'warped', '512_15Trans0.1Scale30Rot','image_pair', f'{pair_id}_opt.tif')
        infrared_path = osp.join(self.root_dir, self.mode, 'warped','512_15Trans0.1Scale30Rot', 'image_pair', f'{pair_id}_sar_warped.tif')
        flow_path = osp.join(self.root_dir, self.mode, 'warped', '512_15Trans0.1Scale30Rot','truth_flow', f'{pair_id}.flo')
        '''
        '''
        visible_path = osp.join(self.root_dir, 'photo',f'{pair_id}_opt.tif')
        infrared_path = osp.join(self.root_dir,'photo',f'{pair_id}_sar_warped.tif')
        flow_path = osp.join(self.root_dir,'flow',f'{pair_id}.flo')
        '''
