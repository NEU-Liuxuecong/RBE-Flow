...
##############  ↓  RBE Pipeline  ↓  ##############
_CN.RBE = CN()
_CN.RBE.RESOLUTION = (8, 2)  # options: [(8, 2)]
_CN.RBE.FINE_WINDOW_SIZE = 5  # window_size in fine_level, must be odd
_CN.RBE.MEDIUM_WINDOW_SIZE = 3  # window_size in fine_level, must be odd

# 1. RBE-backbone (local feature CNN) config
_CN.RBE.RESNET = CN()
_CN.RBE.RESNET.INITIAL_DIM = 128
_CN.RBE.RESNET.BLOCK_DIMS = [128, 196, 256]  # s1, s2, s3

# 2. RBE-coarse module config
_CN.RBE.COARSE = CN()
_CN.RBE.COARSE.INFERENCE = INFERENCE
_CN.RBE.COARSE.D_MODEL = 256
_CN.RBE.COARSE.D_FFN = 256
_CN.RBE.COARSE.NHEAD = 8
_CN.RBE.COARSE.LAYER_NAMES = ['self', 'cross'] * 4
_CN.RBE.COARSE.ATTENTION = 'linear'  # options: ['linear', 'full']


# 3. Coarse-Matching config
_CN.RBE.MATCH_COARSE = CN()
_CN.RBE.MATCH_COARSE.INFERENCE = INFERENCE
_CN.RBE.MATCH_COARSE.D_MODEL = 256
_CN.RBE.MATCH_COARSE.THR = 0.3
_CN.RBE.MATCH_COARSE.BORDER_RM = 2
_CN.RBE.MATCH_COARSE.MATCH_TYPE = 'dual_softmax'  # options: ['dual_softmax']
_CN.RBE.MATCH_COARSE.DSMAX_TEMPERATURE = 0.1
_CN.RBE.MATCH_COARSE.TRAIN_COARSE_PERCENT = 0.2  # training tricks: save GPU memory
_CN.RBE.MATCH_COARSE.TRAIN_PAD_NUM_GT_MIN = 200  # training tricks: avoid DDP deadlock

# 4. RBE-fine module config
_CN.RBE.FINE = CN()
_CN.RBE.FINE.DENSER = False # if true, match all features in fine-level windows
_CN.RBE.FINE.INFERENCE = INFERENCE
_CN.RBE.FINE.DSMAX_TEMPERATURE = 0.1
_CN.RBE.FINE.THR = 0.1
_CN.RBE.FINE.MLP_HIDDEN_DIM_COEF = 2 # coef for  mlp hidden dim (hidden_dim = feat_dim * coef)
_CN.RBE.FINE.NHEAD_FINE_LEVEL = 8
_CN.RBE.FINE.NHEAD_MEDIUM_LEVEL = 7


# 5. RBE Losses

_CN.RBE.LOSS = CN()
_CN.RBE.LOSS.FOCAL_ALPHA = 0.25
_CN.RBE.LOSS.FOCAL_GAMMA = 2.0
_CN.RBE.LOSS.POS_WEIGHT = 1.0
_CN.RBE.LOSS.NEG_WEIGHT = 1.0

# -- # coarse-level
_CN.RBE.LOSS.COARSE_WEIGHT = 0.5
# -- # fine-level
_CN.RBE.LOSS.FINE_WEIGHT = 0.3
# -- # sub-pixel
_CN.RBE.LOSS.SUB_WEIGHT = 1 * 10**4
...
_CN.TRAINER.SB_SUBSET_SAMPLE_REPLACEMENT = True  # Whether to sample each scene with replacement.
_CN.TRAINER.SB_SUBSET_SHUFFLE = True  # Whether to reshuffle samples within each epoch.
_CN.TRAINER.SB_REPEAT = 1  # Repeat each sampled item N times during training.
# 'random' config
...
_CN.RBE.PRETRAIN_PATCH_SIZE = _CN.PRETRAIN.PATCH_SIZE 
...
def get_cfg_defaults(inference=False):
    ...
    if inference:
        _CN.RBE.COARSE.INFERENCE = True
        _CN.RBE.MATCH_COARSE.INFERENCE = True
        _CN.RBE.FINE.INFERENCE = True
    return _CN.clone()
