...
def read_flow(path):
    with open(path,'rb') as f:
        ...
        flow = torch.from_numpy(flow).float()  # Convert to float tensor.
    return flow
...
# def generate_pair_list(root_dir, mode, output_path):
...
#     """
#     # Collect all files under the image_pair directory.
    
#     pair_dir = osp.join(root_dir, mode, 'warped','512_30Trans0.1Scale10Rot', 'image_pair')
#     flow_dir = osp.join(root_dir, mode, 'warped', '512_30Trans0.1Scale10Rot','truth_flow')
#     '''
    
#     pair_dir = osp.join(root_dir, mode, 'warped','512_0Trans0.1Scale90Rot', 'image_pair')
#     flow_dir = osp.join(root_dir, mode, 'warped', '512_0Trans0.1Scale90Rot','truth_flow')
#     '''
#     '''
#     pair_dir = osp.join(root_dir, mode, 'warped','512_30Trans0.3Scale10Rot', 'image_pair')
#     flow_dir = osp.join(root_dir, mode, 'warped', '512_30Trans0.3Scale10Rot','truth_flow')'''
#     '''
#     pair_dir = osp.join(root_dir, mode, 'warped','512_0Trans0Scale90Rot', 'image_pair')
#     flow_dir = osp.join(root_dir, mode, 'warped', '512_0Trans0Scale90Rot','truth_flow')
#     '''
#     # Collect all opt images.
#     opt_files = [f for f in os.listdir(pair_dir) if f.endswith('_opt.tif')]
    
#     # Extract the pair ID and verify the matching files exist.
#     valid_pairs = []
#     for opt_file in sorted(opt_files):
#         pair_id = opt_file[:-8]  # Remove '_opt.tif'.
#         sar_file = f"{pair_id}_sar_warped.tif"
#         flow_file = f"{pair_id}.flo"
        
#         # Check that the corresponding SAR image and flow file exist.
