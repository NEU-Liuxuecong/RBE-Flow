...
from src.rbe.utils.geometry import coords_grid
...
def mask_border_flow(flow, b: int):
    """Apply a border mask to a flow field.
    Args:
...
def mask_border_with_padding(m, bd, v, p_m0, p_m1):  # Fill the border region of the mask.
    if bd <= 0:
        return
    ...
    # Case: 2D mask.
    if p_m0.dim() == 2:
        # Single-mask case; convert it to batch dimension.
...
class CoarseMatching(nn.Module):
    def __init__(self, config):
        ...
        # Config for training the fine-level RBE.
    ...
    def forward(self, feat_c0, feat_c1, data, mask_c0=None, mask_c1=None):
        ...
        flow_c, prob = global_correlation_softmax(
            feat_c0_reshaped, feat_c1_reshaped, self.temperature)
        #print(f"[DEBUG] flow_c generated: shape={flow_c.shape}, mean={flow_c.mean().item():.6f}, max={flow_c.abs().max().item():.6f}")
        
        # Apply border handling to the flow.
        flow_c = mask_border_flow(flow_c, self.border_rm)
        
        #print(f"[DEBUG] flow_c after border processing: mean={flow_c.mean().item():.6f}, max={flow_c.abs().max().item():.6f}")
...
def global_correlation_softmax(feature0, feature1, temperature, pred_bidir_flow=False):
    # global correlation
    b, c, h, w = feature0.shape #bs 256 32 32
    feature0 = feature0.view(b, c, -1).permute(0, 2, 1)  # [B, H*W, C] bs 1024 256
    feature1 = feature1.view(b, c, -1)  # [B, C, H*W]

    correlation = torch.matmul(feature0, feature1).view(
        b, h, w, h, w) / temperature  # [B, H, W, H, W] similarity via matrix multiplication.
    ...
    prob = F.softmax(correlation, dim=-1)  # [B, H*W, H*W] 8 1024 1024
    correspondence = torch.matmul(prob, grid.to(prob.dtype)).view(b, h, w, 2).permute(0, 3, 1, 2)  # [B, 2, H, W] for 16 .to(prob.dtype)

    # when predicting bidirectional flow, flow is the concatenation of forward flow and backward flow
    flow = correspondence - init_grid

    return flow, prob#[16,2,8,8] [16,64,64]
